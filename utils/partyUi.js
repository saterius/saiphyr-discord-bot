...
function buildPartyActionRows(party) {
  ...
  const changeClassDisabled = ![PARTY_STATUS.RECRUITING, PARTY_STATUS.PENDING_CONFIRM].includes(party.status)
  ...
  const primaryRow = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder()
      .setCustomId(`party:change_class:start:${party.id}`)
      .setLabel("เปลี่ยนอาชีพ")
      .setStyle(ButtonStyle.Secondary)
      .setDisabled(changeClassDisabled),
    new ButtonBuilder()
  ...
  const secondaryRow = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder()
      .setCustomId(`party:activate_now:${party.id}`)
      .setLabel("เปิดปาร์ตี้ทันที")
      .setStyle(ButtonStyle.Success)
      .setDisabled(activateNowDisabled),
    new ButtonBuilder()
